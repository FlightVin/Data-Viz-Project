...
export default function ImpactSpider(props) {
    ...
    const [typearray, SetTypeArray] = useState(['Earthquake', 'Wildfire', 'Storm', 'Extreme temperature ', 'Drought', 'Landslide', 'Volcanic activity', 'Epidemic', 'Insect infestation'])
    ...
    useEffect(() => {
        const getdisasterdata = async () => {
            try {
                ...
                const disasterdata1 = disasterdata.filter(d => d["Declaration"] === "Yes" || d["Declaration"] === "No");
                console.log(disasterdata1)
                const disasterdata2 = disasterdata1.filter(d => d["Type"] === type);


                // count of yes and no for each magnitude
                const countYes = d3.rollup(disasterdata2, v => d3.sum(v, d => d.Declaration === "Yes"), d => d.Magnitude);
                const countNo = d3.rollup(disasterdata2, v => d3.sum(v, d => d.Declaration === "No"), d => d.Magnitude);

                // count of total occurrences for each magnitude
                const countTotal = d3.rollup(disasterdata2, v => v.length, d => d.Magnitude);

                // combine the above counts for each magnitude
                const magCounts = Array.from(countTotal.entries()).map(([magnitude, totalCount]) => {
                    const yesCount = countYes.get(magnitude) || 22;
                    const noCount = countNo.get(magnitude) || 0;
                    return { magnitude: (type === "Earthquake" && magnitude === 0) ? 2 : magnitude, yesCount, noCount, totalCount };
                });
    ...
    useEffect(() => {
        if (disasterdata) {
            ...
            svg.append("g")
                .attr("transform", "translate(" + (cfg.margin.left + 1) + "," + cfg.h + ")")
                .call(xAxis);
            ...
            svg.append("g")
                .attr("transform", "translate(" + cfg.margin.left + "," + (cfg.margin.top - 20) + ")")
                .call(yAxis)
                .selectAll("text")
                .attr("font-size", "16px");
                    ...
                    circlesGroup.append('circle')
                        ...
                        .on("mouseover", function (event, d) {
                            d3.select(this)
                                .attr("r", 10)
                            tooltip
                                .attr('x', xScale(x) + cfg.margin.left + 20)
                                .attr('y', yScale(i) + cfg.margin.top - 24)
                                .html(`Magnitude: ${x}` + <br /> + `Emergency declared:${yescount}` + <br /> + `Not Declared: ${nocount}` + <br /> + `Total Occurrences: ${yescount + nocount}`)
                ...
                for (let i = 0; i < nocount; i++) {
                    circlesGroup.append('circle')
                        ...
                        .on("mouseover", function (event, d) {
                            ...
                            tooltip
                                .attr('x', xScale(x) + cfg.margin.left + 20)
                                .attr('y', yScale(i + yescount) + cfg.margin.top - 24)
            ...
            const legend = svg.append("g")
                .attr("transform", "translate(" + cfg.w / 1.2 + "," + (cfg.margin.top + 10) + ")")
                .selectAll("g")
            ...
            legend.append("circle")
                ...
                .attr("cy", (d, i) => 10 + i * 20)
                .attr("r", 6)
                .style("fill", d => d === "Emergency was declared" ? "red" : "green");
